...
#pragma comment(lib, "winmm.lib") // timeBeginPeriod용
...
const float DASH_IMPULSE = 30.0f;
const float DASH_DURATION = 0.2f;
const float DASH_COOLDOWN_TIME = 1.5f;
const float CENTER_RADIUS = 19.0f;
const float BRIDGE_WIDTH = 5.0f;
const float BRIDGE_LENGTH = 40.0f;
...
void UdpReceiveThread() {
    ...
    std::cout << "[Server] UDP Receiver Started." << std::endl;
...
void TcpAcceptThread(SOCKET tcpSocket) {
    std::cout << "[Server] Matchmaking Ready. (TCP: " << TCP_PORT << ", UDP: " << UDP_PORT << ")" << std::endl;
    ...
    while (true) {
        ...
        if (newPlayerId != -1) {
            std::cout << "[Server] Client Connected. ID: " << newPlayerId << std::endl;
        }
        else {
            std::cout << "[Server] Room is full. Connection rejected." << std::endl;
        }
    }
}

void GameLoop() {
    const float dt = 1.0f / 60.0f; // 60FPS 기준
...
int main() {
    ...
    // Sleep() 정확도 향상을 위해 타이머 해상도 1ms로 변경
    timeBeginPeriod(1);
    ...
    std::cout << "=== Dedicated Server Started ===" << std::endl;
    std::cout << "Timer Resolution: 1ms" << std::endl;
